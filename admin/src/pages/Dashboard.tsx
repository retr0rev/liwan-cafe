        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            {recent.items.length === 0 && (
              <p className="text-sm text-chocolate/50">No recent edits</p>
            )}
            {recent.items.map((i) => (
              <p key={i.id} className="border-b border-green/5 py-1 text-sm text-chocolate/80">
                {i.name_en}
              </p>
            ))}
          </div>
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            {recent.categories.map((c) => (
              <p key={c.id} className="border-b border-green/5 py-1 text-sm text-chocolate/80">
                {c.name_en}
              </p>
            ))}
          </div>
